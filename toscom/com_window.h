 ...
 *
 * 特殊キー
 *   入力で使用する特殊キーについて com_custom.h にてキーコードのマクロ宣言を
 *   記述している。これを com_custom.h に記述することにした、最初のきっかけは
 *   Linuxでは BSキーが使えたのに、Cygwinでは使えないという問題からだった。
 *   確認したところ、Linuxと Cygwinでは BSを押した時のキーコードに差分があり、
 *   Linuxの宣言では BSキーが動作しなかった。現状では BSキー以外は問題ないが
 *   環境によって変わる可能性がゼロではないことから、必要に応じてユーザーが
 *   カスタマイズできるヘッダファイルでマクロを宣言することとした。
 *   com_inputWindow()を使用して、特殊キーが効かない時は com_custom.h にある
 *   COM_KEY～ という名前のマクロの宣言値を変更して対応する必要がある。
 *
 *
 * 入力できる文字サイズ(1)
 ...
 *
 *   また、com_inputWindow()でも com_setIntrInputWindow()でキー割込を登録可能。
 *   Enter・BackSpace・ESC については、文字列入力時に特殊な意味があるが、
 *   それ以外の特殊キーに何らかの機能を与えられる。入力内容についても
 *      com_getInputWindow()
 *      com_setInputWindow()
 *      com_cancelInputWindow()
 *   といったI/Fで干渉することが可能になっているので、I/F説明を確認して欲しい。
 */
 ...
 *   マルチスレッドで使用することは想定していない。
 * ===========================================================================
 * iModeでウィンドウモードのデバッグ出力有無を設定する。
 * trueにした場合、ウィンドウモードでの挙動をデバッグログに残す。
 * キー操作などの細かいログも残すことから、かなりの分量になる恐れがあるため
 * 問題解析など特殊な理由がない限りは falseのママにすることを推奨する。
 * デフォルトで falseになっている。
 *
 * デバッグログはファイルにのみ出力する(ウィンドウモード時、標準出力は抑制されて
 * いるため)ので、デバッグログ生成なし(makefileで -DUSEDEBUGLOG未指定)だと、
 * ログが何も残らないことに注意すること。
 */
