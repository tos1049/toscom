 ...
 *   makefileで、MAKEFILE_COMMAND でマクロ指定した文字列を渡す。
 *   (実行用ファイル名になる想定)
 *   マクロ未指定時は空文字""が返る。 
 * ---------------------------------------------------------------------------
 *   エラーは発生しない。
 ...
 *   マルチスレッドで起動することは想定していない。
 * ===========================================================================
 * makefileでは COMMAND という変数を冒頭で定義しているので、本I/Fの出力を
 * 変更したい時は、この変数の定義を修正して、再ビルドすること。
 * この変数の内容を、コンパイル時に -DMAKEFILE_COMMAND="～" というオプションで
 * 指定することにより、アプリ名として設定される。
 *
 * -DMAKEFILE_COMMAND の指定がなかったときは、空文字 "" を返す。
 * このコンパイルオプションの設定は makefileでは別の箇所で RELOPTに対する
 * 内容追加の際に実施しており、その部分を修正しない限り、指定削除はされない。
 *
 * 関数定義は com_spec.c にて実施している。
 * これはアプリ名自体がユーザー定義可能な固有情報であるため。
 */
const char *com_getAplName( void );

/*
 * バージョン取得  com_getVersion()
 *   makefileで、MAKEFILE_VERSION でマクロ指定した文字列を返す。
 *   マクロ未指定時は "0" を返す。
 * ---------------------------------------------------------------------------
 *   エラーは発生しない。
 * ===========================================================================
 *   マルチスレッドで起動することは想定していない。
 * ===========================================================================
 * makefileでは VERSION という変数を冒頭で定義しているので、本I/Fの出力を
 * 変更したい時は、この変数の定義を修正して、再ビルドすること。
 * この変数の内容を、コンパイル時に -DMAKEFILE_VERSION="～" というオプションで
 * 指定することにより、バージョンとして設定される。
 *
 * -DMAKEFILE_VERSION の指定がなかったときは、空文字 "" を返す。
 * このコンパイルオプションの設定は makefileでは別の箇所で RELOPTに対する
 * 内容追加の際に実施しており、その部分を修正しない限り、指定削除はされない。
 *
 * 関数定義は com_spec.c にて実施している。
 ...
 * ---------------------------------------------------------------------------
 *   文字列変換のための com_strdup()によるエラー (大文字小文字区別しない場合)
 * ===========================================================================
 *   mutexによる排他処理が動作するためスレッドセーフとなる。
 * ===========================================================================
 ...
 *
 * 処理のためにメモリの動的捕捉を実施する。捕捉したメモリは処理終了時に自動で
 * 全て解放するため、使う側がメモリ管理をする必要はないが、メモリ捕捉を行う、
 * こと自体を避けたいときは、少し機能縮小するが、com_seekDir2() を使うという
 * 選択肢もある。
 *
...
//＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊
//
// Cygwin では現状 以下で示すスレッド関連I/Fの大半が使用できない。
// (mutex関連の com_mutexLock() と com_mutexUnlock() は問題なく動作する)
//
// スレッド動作中は /proc/<pid>/task というディレクトリが生成され、
// その中にスレッドの pidが入っていることを期待する。
// 上記ディレクトリの内容を走査して、走行中の子スレッド数を確認している。
//
// このディレクトリが見つからない場合、toscomは異常終了する。
// (コアを吐くのではなく com_errorExit() で プログラム実行を終了する)
//
 ...
 *   mutexによる排他処理が動作するためスレッドセーフとなる。
 * ===========================================================================
 * ＊＊ Cygwinでは現状使用不可 (com_createThread()前の説明記述を参照 ＊＊
 *
 ...
 * そのシーケンス番号とそれ以降のファイルオープンで NGを返すようになる。
 * (つまり 0 か 1 を指定すると、最初から NGになる)
 */
...
/*
 * エラーメッセージ出力ON/OFF  com_notifyError()
 * ---------------------------------------------------------------------------
 *   エラーは発生しない。
 * ===========================================================================
 *   データをスレッドごとに持っており、スレッドセーフとなる。
 * ===========================================================================
 * 前述のエラー出力I/Fによる画面出力をON/OFF…する。
 * iDisp は画面出力ON/OFFの指定、iLogはログ出力ON/OFFの指定となる。
 * どちらも trueなら出力ON、falseなら出力OFFとなる。
 *
 * あくまでエラーメッセージ出力のON/OFFなので、エラーカウントは実施する。
 *
 * 本I/Fによる変更前は iDisp/iLog どちらも true に設定された状態となっている。
 *
 * 主に独自のエラーメッセージやエラー処理を実施したいときに使用するI/Fとなる。
 * toscomの他I/Fを使った時に、そのI/F内で発生するエラーメッセージを敢えて
 * 抑制したい時にも、この設定変更を利用する。
 * いずれにしても、ユーザー独自の方法で何らかのエラー通知を出すのが望ましい。
 */
 ...
 *   マルチスレッドについては考慮済み。
 * ===========================================================================
 * makefile の NAMELIST で指定されたネームリストが生成されている場合に
 ...
 * コンパイルオプションを指定していなくても使用可能。NAMELISTで宣言された
 * 名前で所定のネームリストが作られていれば、いつでも機能する。
 * ネームリストを作る nmコマンドの内容は com_dispFuncTrace()の説明を参照。
 */
 ...
 * ビルド時に .namelist.toscom というファイルを作る。これは
 *     nm -nl (実行用ファイル) > .namelist.toscom
 * というコマンド実行結果で、これがあればアドレスから関数名とファイル位置を
 * 取得できるようになる。ファイル名は makefileの NAMELISTで定義する。
 *
...
/*
 * デバッグ時の各種アサート関数  com_assert*() <一覧は下記説明参照>
 *   いずれも返り値はなし。
 ...
 *   com_assertEquals()
 *   com_assertNotEquals()
 *     iExpectedと iResultは long型(符号あり数値)。
 ...
 *     iExpectedと iResultは ulong型(符号なし数値)。
 ...
 *     iExpectedと iResultは float型(単精度浮動小数点数値)。
 ...
 *     iExpectedと iResultは double型(倍精度浮動小数点数値)。
 ...
 *   com_assertString()
 *   com_assertStringLen()
 *     iExpectedと iResultは char*型(文字列)。
 ...
 *   com_assertNull()
 *   com_assertNotNull()
 *     iExpectedは引数として持たず、iResultは void*型(ポインタ値)。
 *     iResultが NULL または 非NULL かを確認。
 *
 *   com_assertEqualsP()
 *   com_assertNotEqualsP()
 *     iExpectedと iResultは void*型(ポインタ値)。
 *     com_assertEqualsP()は iExpected と iResult が等しいことを確認。
 *     com_assertNotEqualsP()は iExpected と iResult が等しくないことを確認。
 */
 ...
 *  void com_assertEqualsP( char *iLabel, void* iExpected, void* iResult );
 *  void com_assertNotEqualsP( char *iLabel, void* iExpected, void* iResult );
 */
...
#define com_assertEqualsP( LABEL, EXPECTED, RESULT ) \
    com_assertEqualsPFunc( LABEL, (EXPECTED), (RESULT), COM_FILELOC )
#define com_assertNotEqualsP( LABEL, EXPECTED, RESULT ) \
    com_assertNotEqualsPFunc( LABEL, (EXPECTED), (RESULT), COM_FILELOC )
