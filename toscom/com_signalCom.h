 ...
 *
 *   個別I/Fには幾つか書き方のフォーマットがあるので、それを下記に示す。
 *
 * (1)解析I/F
 *
 ...
 *   com_inheritRasData()
 *     result(解析結果)が trueだった時、ioHead->ras を ioHead->next.stack[] に
 *     継承する。ただし解析されたプロトコルと一致した時は、解析のために使用済み
 *     となるので何もしない。
 *
 *   com_rollbackSignal()
 ...
 *
 * com_addPrm()は入力から oTarget->list[]を以下のように追加する。
 ...
 * 
 * com_addPrmDirect()は入力から oTarget->list[]を以下のように追加する。
 *   .tagBin.top : NULL
 *   .tagBin.len : 0
 *   .tag        : iTag      (実際のタグ値)
 *   .lenBin.top : NULL
 ...
 *
 * com_addPrmTlv()は iPrmの内容を oTarget->list[] に直接追加する。
 */
BOOL com_addPrm(
 ...
 * 事態が発生する可能性がある。
 *
 * 本I/Fで編集した oPrmは com_dispAsnPrm()でデコード出力が可能。
 */
...
// デコードI/Fの作成方針については COMSIGCOMANLZ の直前に記述した
// ガイドラインを確認すること。
 ...
 *   マルチスレッドで影響は考慮済み。
 * ===========================================================================
 * デコード結果を出力するためのI/F群。引数で指定した内容を特定の書式で出力する。
 * どのI/Fでも共通なのは、行頭に # を付与すること。以下にそれ以外の出力内容を
 * 挙げる。
 *
 * com_dispDec()
 *   iFormatとそれ以降で示した文字列を出力、その後改行する。
 *   改行させたくない時は com_printf()を駆使すること。
 *
 * com_dispVal()
 *   (iNameの内容文字列) = (iValueの内容値) を出力、その後改行する。
 *
 * com_dispSig()
 *   (iNameの内容文字列)[iCodeの内容値]  <length=(iSig.lenの値)> を出力後、
 *   改行して、iSig.topから始まり、iSig.lenのデータ長でバイナリダンプする。
 *   iCodeには COM_SIG_* のプロトコル値が入る想定だが、COM_NO_PTYPE を
 *   指定した場合、iCodeの内容値出力を省略できる。
 *
 * com_dispPrm()
 *   (iNameの内容文字列) = (iTopの内容) を出力して改行する。
 *   iSizeで iTopから始まるデータのサイズを指定し、その値によって以下のように
 *   出力方法が変わる。
 *     iSize が 0 の場合、iTopを文字列ポインタとみなし %s で出力する。
 *     iSize が 1～COM_32BIT_SIZE(4) の場合、iTopを long*型数値ポインタとみなし
 *     その内容を 16新数値と 10進数値で並列出力する。
 *     iSize が COM_32BIT_SIZE(4) より大きい場合、iTopを uchar*型バイナリ列の
 *     先頭アドレスとみなし、16進数でそのサイズ分のデータを出力する。
 *
 * com_dispPrmList()
 *   iPrmで指定したパラメータリストを、タグ長 iTagSize、レングス長 iLenSizeで
 *   一覧出力する。 この際 iFuncにペイロードデータ判定関数が指定可能で、
 *   この関数は iTagがペイロードデータを示すタグ値であれば true、
 *   そうでなければ falseを返すように作成する。もしこの関数が trueを返す場合
 *   そのタグで示されたデータはペイロードデータ・・すなわち次プロトコルデータ
 *   とみなすため、本I/Fで内容出力はせず <payload data> と出力するに留める。
 *   こうした判定が不要なら iFuncは NULL指定する。
 *
 * com_dispBin()
 *   (iNameの内容文字列) = (iTopのバイナリ内容) を出力して改行する。
 *   iTopはバイナリ列のアドレスとみなし、iLengthのサイズ分 出力する。
 *   com_dispPrm()で iSizeを 4より大きくしたときと似たことをするが、
 *   本I/Fは iSep (数値間の文字) と iHex (16進数出力するか？) を指定可能。
 *   例えば iLengthが 4で、iSep='.' iHex=false とすると IPv4アドレスになる。
 *   com_dispPrm()の場合 iSep=' ' iHex=true で固定設定しているので、
 *   それ以外の書式で出力したい時や 4オクテット未満でもバイナリ出力したい時に
 *   本I/Fを使用する。
 *
 * com_dispNext()
 *   次プロトコルが何かを出力するための共通I/F。
 *   next protocol = (iProtocolの16進数値と10進数値) -> プロトコル名[iType値]
 *   という出力をする。iSizeは iProtocolのデータサイズ(オクテット数)を指定する。
 *   (プロトコルによって次プロトコルを示す値のサイズも様々のため)
 *   プロトコル名は iTypeの値に対応するものを自動取得する。この iTypeは
 *   COM_SIG_* で示すプロトコル値を想定する。
 */
 ...
 *   フラグメントデータではない場合(iInf->isFragmentが false)、
 *   iInf->sig.ptype の内容をそのまま返す。
 *   フラグメントデータの場合(iInf->isFragmentが true)、
 *   COM_SIG_FRAGMENT を固定で返す。
 * ---------------------------------------------------------------------------
 *   エラーは発生しない。
 * ===========================================================================
 *   マルチスレッドの影響は受けない。
 * ===========================================================================
 * iInfで指定したデータが格納するプロトコル種別(iInf->sig.ptype)を返す。
 * ただし、フラグメントデータである場合(iInf->isFragmentが true)、
 * COM_SIG_FRAGMENT を固定で返す。
 */
long com_getSigType( com_sigInf_t *iInf );

/*
 * デコード名検索  com_searchDecodeName()
 *   検索された文字列を返す。
 *   検索未ヒット時はエラーを出して NULLを返す。
 * ---------------------------------------------------------------------------
 *   COM_ERR_DEBUGNG: [com_prmNG] !iList
 *   COM_ERR_NOTPROTO: 検索未ヒット時のエラー
 * ===========================================================================
 *   マルチスレッドで動作することは想定していない。
 * ===========================================================================
 * iListで示した文字列情報データリストから ,codeが iCode と一致するものを
 * 検索し、その文字列(.name)を返す。数字から文字列への変換に使用する。
 *
 * iAddNumが falseの場合は単純に .nameの内容を返すのみ。
 * trueの場合は .nameの文字列内容に加えて iCodeの値を 16進数と10進数で付記した
 * 文字列を返す。
 * 
 * iListは最後を必ず COM_DECODENAME_END とすること。
 * このマクロだけで、必要な最終データの構造体リテラルとなっているので、
 * {} で囲んだりする必要はない。
 * com_signalSet1.cでも多数使用しているので、例には事欠かないだろう。
 */
 ...
 * ---------------------------------------------------------------------------
 *   COM_ERR_DEBUGNG: [com_prmNG] !iHead || !iSigLabel || !iConType
 * ===========================================================================
 *   マルチスレッドで動作することは想定していない。
 * ===========================================================================
 * com_analyzeTxtBase()で解析したデータ iHead の内容をデコード出力する。
 * 本I/Fを呼ぶだけで、基本的なデコードは完了する。
 *
 * 出力したいヘッダ名が限られている場合、その文字列リストを iHdrListで指定する。
 * このリストの最終データは必ず NULL にすること。この場合、信号データから、
 * リストに含まれるヘッダだけを抽出して、ヘッダ名と内容を出力する。
 * iHdrListを NULL指定した場合、全ヘッダ出力となる。
 *
 * iSigLabelにはメソッド名の文字列、
 * iSigTypeには iSigLabelに対応する種別値を設定する。(予め取得が必要) 
 * iSigTypeが COM_SIG_METHOD_END(99)以下の場合、リクエストメソッドと判断し、
 * 信号名には iSigLabel の文字列を要する。COM_SIG_METHOD_ENDより大きい場合
 * レスポンスと判断し、iSigTypeの値そのものをステータスコードとして使う。
 *
 * iConTypeにはボディの種別を示すヘッダ名の文字列を指定する。
 * 例えば SIP であれば "Content-Length" であり COM_CAP_SIPHDR_CLENGTH で
 * 指定すれば問題ない。
 */
...
/*
 * ASN.1プロトコルのパラメータ出力共通処理  com_dispAsnPrm()
 * ---------------------------------------------------------------------------
 *   COM_ERR_DEBUGNG: [com_prmNG] !iPrm || !iLabel
 * ===========================================================================
 *   マルチスレッドで動作することは想定していない。
 * ===========================================================================
 * com_getAsnTlv()で解析したパラメータリストを iPrmに指定して、内容出力する。
 *
 * iLabelは行頭に付与する文字列を指定する。
 * その後に タグ・レングス・バリューを一つずつ出力していく。
 * Constructor Formの場合、その配下はインデントを下げて出力する。
 *
 * iStartはリストの何番目から出力するかを指定する(先頭からなら 0)。
 */
void com_dispAsnPrm(
...
/*
 * 大きなサイズのデータを分割する仕組みを持つプロトコルが存在する。
 * 例えば IPフラグメント、TCPセグメンテーション 等が挙げられるだろう。
 * toscomではこうしたデータの管理を一元で行う。
 *
 * データ分割が行われる場合、プロトコルごとにID値が何か用意される。
 * これらと共に送信元/送信先の情報も必要なら保持して、キー情報をまず作成する。
 * これは下記する「分割条件データ」のことである。
 *
 * それから、com_stockFragments()で分割された実際のデータを一つずつ保持する。
 * 分割の終わりをどのように判定するかはプロトコルによって異なるが、
 * 「分割の最終データが分かるようになっている(IP)」とか、
 * 「データサイズが最初に与えられ、分割データがそのサイズを満たせば終了(TCP)」
 * など。プロトコル固有の判断材料については、分割条件データの usr1・usr2 を
 * 駆使して保持し、判定に使用すると良い。
 *
 * 分割データが揃ったら、保持したデータを順番に従って結合する。
 * 結合の処理自体はユーザーが作成する必要がある。
 * (IPの場合は位置が信号内に含まれる。TCPは順番を保証するので受けた順で良い)
 * 保持した分割データの取得は com_searchFragment()で行う。
 *
 * 結合が終わったら、com_freeFragments()で保持したデータをメモリ解放すること。
 */
...
/*
 * 分割条件データ解放  com_freeSigFrgCond()
 * ---------------------------------------------------------------------------
 *   COM_ERR_DEBUG: [com_prmNG] !oTarget
 * ===========================================================================
 *   マルチスレッドiの影響は考慮済み。
 * ===========================================================================
 * oTargetで指定した分割条件データの送信元/送信先のデータを解放する。
 */
void com_freeSigFrgCond( com_sigFrgCond_t *oTarget );

/*
 * 分割データ保持  com_stockFragments()
 *   実際に保持した分割データ情報のアドレスを返す。
 *   処理NG時は NULLを返す。
 * ---------------------------------------------------------------------------
 *   COM_ERR_DEBUG: [com_prmNG] !iCond || !iFrag
 * ===========================================================================
 *   マルチスレッドiの影響は考慮済み。
 * ===========================================================================
 * iCondは予め作成した分割条件データのアドレスを指定する。
 * 分割条件データ作成のガイドラインは以下のようになる：
 *   .type: プロトコル種別値(どのプロトコルの分割データか区別する)
 *   .id:   どのデータの分割かを識別する、プロトコルごとに定められた数値
 *   .src:  送信元情報 (必要なら)
 *   .dst:  送信先情報 (必要なら)
 *   .us41: ユーザー情報1 (必要なら)
 *   .usr2: ユーザー情報2 (必要なら)
 * プロトコルによっては .type と .id だけでデータを識別できるかもしれない。
 * それが保証できない時に 送信元/送信先の情報を保持する。信号データから
 * この情報を保持する時は、そのデータのポインタアドレスをそのまま格納しても
 * うまくいかない(解析データを全て保持するなら良いが、それは非推奨)。
 * そこで com_copySigBin()を使うなどして、メモリを動的に確保してコピーすること。
 * 当然その開放が必要になるが、そのために com_freeSigFrgCond()がある。
 * それ以外にも保持したいものがある場合はユーザー情報1・2を駆使する。
 * 使用しない情報はオール0にすることを強く推奨する。
 * 
 * iSegは信号の配置順を示す任意の値となる。この値はデータを保持するとき
 * com_sigFrg_t型の .inf[].seg にそのまま設定するが、それをどのように使うかは
 * 呼び元に委ねられる(使わなくても良い)。例を上げておくと、IPフラグメントでは
 * フラグメントオフセット値(＝全体でのデータ位置)を入れて、結合時に使う。
 *
 * iFragは実際の分割データで、
 * iFrag->topがその先頭アドレス、iFrag->lenがそのサイズと認識される。
 * ここで渡されたアドレスをそのまま保持するのではなく、実際のデータをコピーして
 * 保持する。
 */
 ...
 *   見つからなかった場合は NULLを返す。
 * ---------------------------------------------------------------------------
 *   COM_ERR_DEBUG: [com_prmNG] !iCond
 * ===========================================================================
 *   マルチスレッドiの影響は考慮済み。
 * ===========================================================================
 * iCondと内容が合致する条件で保持された分割データのアドレスを返す。
 * 条件合致する分割データが存在しない時は NULLを返す。
 *
 * 返されたデータの内容のうち .data.cnt が既に保持している分割データ数、
 * .data.inf[] が個々の分割データとなる。
 * .data.segMax や .data.ext は処理の中で自由に使用して良い。特に後者は
 * メモリの動的確保を必要としているが、解放は com_free()を使うので、
 * あまり複雑なデータ構造にはしないことをお勧めする。
 *
 * 全て揃って結合したら、データ保持は不要になるので、本I/Fで取得したアドレスに
 * 対して、com_freeFragments()でデータ解放すること。
 */
...
/*
 * 分割データ解放  com_freeFragments()
 * ---------------------------------------------------------------------------
 *   COM_ERR_DEBUG: [com_prmNG] !iCond
 * ===========================================================================
 *   マルチスレッドiの影響は考慮済み。
 * ===========================================================================
 * iCondで指定された条件と合致する分割データをメモリ解放する。
 *
 * 解放するのは保持したデータ部部のみで、管理データ自体は「未使用」の状態で
 * そのまま残し、別の分割データ保持の際に再利用する。本I/Fで「未使用」に
 * しない限り、その管理データが別の分割データ保持に使われることはない。
 * 管理データのメモリ捕捉/解放は内部で実施するので、呼ぶ側は意識不要。
 */
